  ...
  const toggleUserMenu = () => {
   ...
   const [Produit ,setProduit] = useState([]);
      ...
      const deleteP = async (pk) =>{
        try {
          const response = await fetch(`http://localhost:8000/produit/${pk}/`,{
            method: "DELETE",
          });
        const data =  await response.json();
                            ...
                            <div className="w-full h-full">
                                <div className="h-1/4  p-5  flex flex-col lg:flex-row gap-2 items-center text-xs lg:text-base">
                                    <div className="w-full">
                                        <div className="font-semibold text-lg lg:text-2xl">Utilisateurs</div>
                                        <div>Liste des employer dans le societer  </div>
                                    </div>
                                ...
                                <div className="hidden lg:block p-5 w-full h-full text-sm">
                                    <table className="p-5 w-full "> 
                                        <thead className="border-b ">
                                            <tr className="">
                                                <th className="font-bold text-start py-2 pr-2 w-1/12">Matricule</th>
                                                <th className="font-bold text-start py-2 pr-2 w-2/12">Nom </th>
                                                <th className="font-bold text-start py-2 pr-2 w-2/12">Prenoms</th>
                                                <th className="font-bold text-start py-2 pr-2 w-1/12">Poste</th>
