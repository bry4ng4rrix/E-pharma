  ...
  const toggleUserMenu = () => {
   ...
   const [Produit ,setProduit] = useState([]);
      ...
      const deleteP = async (pk) =>{
        try {
          const response = await fetch(`http://localhost:8000/api/vente/${pk}/`,{
            method: "DELETE",
          });
        const data =  await response.json();
         setProduit((prev) => prev.filter((util) => util.id !== pk));
         redirect("/admin/vente")
        }
        catch (err){
        }
      }
                            ...
                            <div className="w-full h-full">
                                <div className="h-1/4  p-5  flex flex-col lg:flex-row gap-2 items-center text-xs lg:text-base">
                                    <div className="w-full">
                                        <div className="font-bold text-lg lg:text-2xl ">Vente</div>
                                        <div>List des vente par mois  </div>
                                    </div>
                                ...
                                <div className="hidden lg:block p-5 w-full h-full text-sm">
                                    <table className="p-5 w-full "> 
                                        <thead className="border-b ">
                                            <tr className="">
                                                <th className="font-bold text-start py-2 pr-2 w-1/12">Noms</th>
                                                <th className="font-bold text-start py-2 pr-2 w-1/12">Prix Total</th>
                                        ...
                                        <tr className="border-b">
                                                <td className="py-3">
                                                    <div className="flex gap-2">
                                                        ...
                                                        <div>
                                                            <div className="font-medium">{util.produit_nom}</div>
                                                            <div className="text-gray-400">Nombre : {util.quantite}</div>
                                                ...
                                                <td>
                                                <div className="w-fit bg-blue-200  px-2  py-1 text-green-900 font-semibold border  border-blue-400 rounded-lg">{util.prixtotale} <span>Ar</span></div>
            
                                                </td>
                                                <td>
                                                    <div className='font-semibold'>  {util.date}</div>
                                                </td>
                                                <td>
                                        ...
                                        <div className="border rounded-lg flex items-center p-2 gap-2 ">
                                            <div className="w-1/4">
                                                ...
                                                <div className="w-3/4 ">
                                                    ...
                                                    <div className="text-gray-300">Nombre : {util.Nombre}</div>
                                                        <div className="grid grid-cols-2 gap-2">
                                                            ...
                                                            <div className="w-fit bg-blue-200  px-2  py-1 text-xs text-green-900 font-semibold border  border-blue-400 rounded-lg">500 <span>Ar</span></div>
